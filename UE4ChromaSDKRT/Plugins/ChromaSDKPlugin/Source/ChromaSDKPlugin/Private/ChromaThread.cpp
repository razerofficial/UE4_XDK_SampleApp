...
void ChromaThread::UseIdleAnimation(EChromaSDKDeviceEnum::Type device, bool flag)
{
	switch (device)
	{
...
void ChromaThread::SetIdleAnimationName(const char* name)
{
	...
	AnimationBase* animation = IChromaSDKPlugin::GetChromaSDKPlugin()->GetAnimationInstanceName(name);
...
void ChromaThread::ProcessAnimations(float deltaTime)
{
	...
	for (int i = 0; i < (int)EChromaSDKDeviceEnum::DE_MAX && _sWaitForExit; ++i)
	{
		if (_sUseIdleAnimation[i] &&
			detectIdle[i])
		{
			AnimationBase* idleAnimation = IChromaSDKPlugin::GetChromaSDKPlugin()->GetAnimationInstanceIfOpenName(_sIdleAnimation[i].c_str());
...
void ChromaThread::Start()
{
	...
	ss << _sThread->get_id();
	string strId = ss.str();
	FString fs(strId.c_str());
	UE_LOG(LogTemp, Log, TEXT("ChromaThread: %s"), *fs);
	*/
...
int ChromaThread::GetAnimationId(int index)
{
	...
	if (index < (int)_sAnimations.size())
	{
		...
		if (animation != nullptr)
		{
			return IChromaSDKPlugin::GetChromaSDKPlugin()->GetAnimationIdFromInstance(animation);
		}
	}
	return -1;
}
